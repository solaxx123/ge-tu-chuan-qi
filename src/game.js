// 成就将在初始化后从localStorage加载
        ...
        function renderAchievements() {
            ...
            if (unlockedCount >= totalAch && !endingUnlocked) {
        ...
        (function() {
            ...
            var pipes = [];
            var score = 0, running = false, won = false, animId = null, lastTime = 0;
            var GRAVITY = 0.0018, JUMP_VEL = -0.58, PIPE_SPEED = 0.15, PIPE_GAP = 150, PIPE_WIDTH = 55, PIPE_SPACING = 340, TARGET = 10;

            function reset() { bird.y = H/2; bird.vy = 0; pipes = []; score = 0; won = false; updateUI(); }
            ...
            function draw() {
                ...
                if (!running && (pipes.length > 0 || won)) {
                    ctx.fillStyle = "rgba(0,0,0,0.55)"; ctx.fillRect(0, 0, W, H);
                    ctx.font = "bold 22px Arial"; ctx.textAlign = "center";
                    if (won) {
                        ctx.fillStyle = "#ffd700";
                        ctx.fillText("🗝️ 成功救出鸽鸽！💕", W/2, H/2);
                    } else {
                        ctx.fillStyle = "#ff6b9d";
                        ctx.fillText("💔 兔兔被抓了！", W/2, H/2);
                    }
                    ctx.textAlign = "start";
                }
            }
            ...
            function win() {
                running = false; won = true; cancelAnimationFrame(animId); playVictoryMusic();
