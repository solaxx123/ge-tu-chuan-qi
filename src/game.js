// 成就将在初始化后从localStorage加载
        ...
        function draw() {
            // 梦幻背景：基于历史最高分逐渐变化
            var dreamLevel = Math.min(1, highScore / 1314);
            ...
            // 梦幻光效叠加
            if (dreamLevel > 0.1) {
                var glow = ctx.createRadialGradient(canvas.width/2, canvas.height/2, 0, canvas.width/2, canvas.height/2, canvas.width*0.7);
                glow.addColorStop(0, 'rgba(255,180,200,'+(dreamLevel*0.08)+')');
            ...
            // 网格线
            ctx.strokeStyle = 'rgba(255, 107, 157, '+(0.08+dreamLevel*0.04)+')';
            ctx.lineWidth = 0.5;
            ...
            // 高分段：背景飘落花瓣
            if (dreamLevel > 0.3 && Math.random() < dreamLevel * 0.3) {
                ctx.font = (10+Math.random()*8)+'px Arial';
